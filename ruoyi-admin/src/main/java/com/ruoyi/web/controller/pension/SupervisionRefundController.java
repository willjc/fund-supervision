...
public class SupervisionRefundController extends BaseController
{
    ...
    public AjaxResult getInfo(@PathVariable("refundId") Long refundId)
    {
        ...
        List<RefundRecord> records = refundRecordService.selectRefundRecordList(query);
        return records.isEmpty() ? AjaxResult.error("退款记录不存在或无权查看")
                : AjaxResult.success(records.get(0));
    }
    ...
    {
        try {
            return toAjax(refundRecordService.approveRefund(refundId, getUsername(), dataScopeUserId()));
        }
        catch (Exception e)
        {
    ...
    public AjaxResult reject(@PathVariable Long refundId, @RequestBody RefundRecord refundRecord)
    {
        try
        {
            return toAjax(refundRecordService.rejectRefund(refundId, getUsername(),
                    refundRecord.getApproveRemark(), dataScopeUserId()));
        }
        catch (Exception e)
        {
            return AjaxResult.error("审批失败：" + e.getMessage());
    ...
    public AjaxResult batchApprove(@RequestBody Long[] refundIds)
    {
        ...
        for (Long refundId : refundIds) {
            try {
                if (refundRecordService.approveRefund(refundId, getUsername(), dataScopeUserId()) == 1) {
                    successCount++;
                } else {
                    failCount++;
                }
    ...
    private void applyDataScope(RefundRecord refundRecord)
    {
        refundRecord.setCurrentUserId(dataScopeUserId());
    }

    private Long dataScopeUserId()
    {
        return Long.valueOf(1L).equals(getUserId()) ? null : getUserId();
    }
}
