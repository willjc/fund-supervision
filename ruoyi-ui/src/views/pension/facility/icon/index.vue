  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="设施类型" prop="facilityType">
        <el-select v-model="queryParams.facilityType" placeholder="请选择设施类型" clearable>
          <el-option
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['pension:facility:icon:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['pension:facility:icon:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['pension:facility:icon:remove']"
        >删除</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="facilityIconConfigList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="设施类型" align="center" prop="facilityType">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.facility_type" :value="scope.row.facilityType"/>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listFacilityIconConfig(this.queryParams).then(response => {
        this.facilityIconConfigList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getFacilityIconConfig(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateFacilityIconConfig(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addFacilityIconConfig(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除设施图标配置编号为"' + ids + '"的数据项？').then(function() {
        return delFacilityIconConfig(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('pension/facility/icon/export', {
        ...this.queryParams
      }, `facilityIconConfig_${new Date().getTime()}.xlsx`)
    },
    // 图标选择器选择图标
    selected(name) {
