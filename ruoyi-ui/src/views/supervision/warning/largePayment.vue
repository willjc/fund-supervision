<template>
  <div class="app-container">
        ...
        <el-card class="box-card">
          <div class="stat-card">
            ...
            <div class="stat-content">
              <div class="stat-title">大额支付预警</div>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleHandle"
          :disabled="single"
          v-hasPermi="['supervision:warning:largePayment:handle']"
        >批量处理</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['supervision:warning:largePayment:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="warningList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="支付金额" align="center" prop="paymentAmount" width="120">
        <template slot-scope="scope">
          <span class="money-text">¥{{ scope.row.paymentAmount.toLocaleString() }}</span>
        </template>
      </el-table-column>
      <el-table-column label="阈值标准" align="center" prop="thresholdAmount" width="120">
        <template slot-scope="scope">
          <span class="money-text">¥{{ scope.row.thresholdAmount.toLocaleString() }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="支付类型" align="center" prop="paymentType" width="100">
        <template slot-scope="scope">
          <el-tag :type="getPaymentTypeTag(scope.row.paymentType)" size="small">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="180">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            v-hasPermi="['supervision:warning:largePayment:notify']"
          >通知</el-button>
        </template>
      </el-table-column>
    </el-table>
      ...
      <el-form ref="handleForm" :model="handleForm" :rules="handleRules" label-width="100px">
        ...
        <el-form-item label="处理结果" prop="handleResult">
          <el-select v-model="handleForm.handleResult" placeholder="请选择处理结果">
            <el-option label="合规支付" value="合规支付" />
            <el-option label="违规已纠正" value="违规已纠正" />
            <el-option label="提供合理解释" value="提供合理解释" />
    ...
    <!-- 详情对话框 -->
    <el-dialog title="大额支付预警详情" :visible.sync="detailOpen" width="700px" append-to-body>
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="支付金额">¥{{ detailForm.paymentAmount ? detailForm.paymentAmount.toLocaleString() : 0 }}</el-descriptions-item>
        <el-descriptions-item label="阈值标准">¥{{ detailForm.thresholdAmount ? detailForm.thresholdAmount.toLocaleString() : 0 }}</el-descriptions-item>
        ...
        <el-descriptions-item label="支付类型">
          <el-tag :type="getPaymentTypeTag(detailForm.paymentType)">{{ detailForm.paymentType }}</el-tag>
        </el-descriptions-item>
        <el-descriptions-item label="收款方">{{ detailForm.payee || '未填写' }}</el-descriptions-item>
        <el-descriptions-item label="支付用途">{{ detailForm.paymentPurpose || '未填写' }}</el-descriptions-item>
        <el-descriptions-item label="预警等级">
          <el-tag :type="getWarningLevelType(detailForm.warningLevel)">
            {{ getWarningLevelText(detailForm.warningLevel) }}
          </el-tag>
        </el-descriptions-item>
        <el-descriptions-item label="预警时间">{{ detailForm.warningTime }}</el-descriptions-item>
        <el-descriptions-item label="处理状态">
          <el-tag :type="getHandleStatusType(detailForm.handleStatus)">
            {{ getHandleStatusText(detailForm.handleStatus) }}
          </el-tag>
        </el-descriptions-item>
    ...
    getList() {
      ...
      const mockData = {
        ...
        rows: Array.from({ length: 10 }, (_, i) => ({
          ...
          paymentAmount: Math.floor(Math.random() * 300000) + 100000,
          thresholdAmount: Math.floor(Math.random() * 100000) + 50000,
          paymentType: ['设备采购', '工程款', '服务费', '材料采购', '租金支付'][Math.floor(Math.random() * 5)],
          payee: ['设备供应商', '建筑公司', '服务公司', '材料供应商', '物业管理公司'][Math.floor(Math.random() * 5)],
          paymentPurpose: '购买养老设备和相关用品',
          ...
          handleResult: Math.random() > 0.5 ? ['合规支付', '违规已纠正', '提供合理解释', '进一步调查'][Math.floor(Math.random() * 4)] : null,
          handleOpinion: Math.random() > 0.5 ? '经核查，该笔大额支付用于购买急需的医疗设备，手续齐全，符合规定。' : null
        }))
      }
      mockData.rows.forEach(item => {
        item.excessAmount = item.paymentAmount - item.thresholdAmount
      })
      ...
      this.statistics = {
        ...
        avgAmount: Math.round(mockData.rows.reduce((sum, item) => sum + item.paymentAmount, 0) / mockData.rows.length)
      }
      this.loading = false
    },
    handleQuery() {
