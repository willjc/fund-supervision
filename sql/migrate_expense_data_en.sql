...
SELECT
    ...
    'system' as create_by,
    oi.create_time,
    'Migrated from order data' as remark
FROM order_info oi
...
SELECT
    ...
    'Deposit Usage' as description,
    da.apply_id as related_id,
    'deposit_apply' as related_type,
    'system' as create_by,
    da.create_time,
    'Migrated from deposit application' as remark
...
-- 显示迁移结果统计
SELECT
    'Migration Statistics' as Item,
    (SELECT COUNT(*) FROM expense_record) as TotalRecords,
    (SELECT COUNT(*) FROM expense_record WHERE expense_type = 'service') as ServiceRecords,
